#!/usr/bin/env python3
"""
Script para traducir automáticamente los casos de STANS ABOGADOS
Usa las mejores prácticas de SEO y organización de archivos
"""
    ...
    def translate_case_content(self, case: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in fields_to_translate:
            if field in translated_case and translated_case[field]:
                translated_case[f"{field}_ar"] = self.translate_text(translated_case[field], 'ar')
        ...
        if 'contenido' in translated_case and translated_case['contenido']:
            ...
            for section, content in translated_case['contenido'].items():
                if content:
                    translated_content[f"{section}_ar"] = self.translate_text(content, 'ar')
